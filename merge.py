...
blob = BlobClient.from_connection_string(conn_string=connectionString, container_name=containerName, blob_name=outputBlobName)
...
df = reduce(lambda left, right : pd.merge(left, right,on=['Référence'], how='outer'), [amazon,apple,microsoft,google,facebook,tesla,zoom])
